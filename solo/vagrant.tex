...
In most cases, your machine will not contain chef client. We should install it before using our kitchen. As you remember, we have knife with command \inline!prepare!. Let use this command:
...
We used some options for the \inline!knife prepare! command. \inline!-i! option specify ssh key for machine, \inline!-N! option specify node name (if it different from host name) and \inline!-p! option specify SSH port. In most cases, this port is 2222, but if you running several machines from vagrant, it will be different. You can read what port is used for SSH by machine from output of command \inline!vagrant up!. All this credentials used to login by ssh on the node and install chef client.
